...
export type CodeRole = 'plain' | 'keyword' | 'string' | 'comment' | 'number';
...
export function highlight(code: string, lang = ''): CodeSpan[] {
  ...
  while ((m = re.exec(code)) !== null) {
    if (m.index > last) spans.push({ text: code.slice(last, m.index), role: 'plain' });
    if (m[1] || m[2]) spans.push({ text: m[0], role: 'comment' });
    else if (m[3]) spans.push({ text: m[0], role: 'string' });
    else if (m[4]) spans.push({ text: m[0], role: 'number' });
    else spans.push({ text: m[0], role: KEYWORDS.has(m[0]!) ? 'keyword' : 'plain' });
    last = re.lastIndex;
    if (re.lastIndex === m.index) re.lastIndex++; // guard against any zero-width match
  }
  if (last < code.length) spans.push({ text: code.slice(last), role: 'plain' });
  return spans;
}
