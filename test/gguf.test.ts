...
import { configuredContextWindow, detectServerContextWindow, ensureGgufServer, stopGgufServers } from '../src/gguf.js';
...
test('context policy reads explicit launcher args and prefers live server metadata', async () => {
  assert.equal(configuredContextWindow(entry({ gguf: '/model.gguf', ggufArgs: ['-c', '49152'] })), 49_152);
  assert.equal(configuredContextWindow(entry({ vllm: 'org/model', vllmArgs: ['--max-model-len=131072'] })), 131_072);
  ...
  try {
    assert.equal(await detectServerContextWindow('http://127.0.0.1:8080/v1'), 65_536);
  } finally {
