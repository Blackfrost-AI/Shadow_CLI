...
> Current build: **`v8.5.0`** — onboarding model discovery: the wizard now probes your endpoint and shows the models your key can actually use, detects adapter compatibility and self-hosted hosting honestly, and saves the picked allowlist as `/model` presets — terminal and web (`shadow onboard --web`). Plus the deep-review hardening line: per-pad-class envelope closure, config-write gate over multi_edit/apply_patch, redirect-aware + fail-closed egress walls, vault merge-verify-shred onboarding, liveness-checked local servers, docker vLLM cleanup, and the MCP/Anthropic/TUI correctness fixes (25 adversarially-verified findings, all fixed and tested). On top of the v8.4.0 UX parity line: first-run auto-onboard, Shift+Tab plan mode, Ctrl+X M model picker, `/export html`, stable `web.token`, auto-format after writes, Claude-session import, spend guardrails, LSP diagnostics after writes, and `/goal` mission mode. Paste-safe input since v8.0.2: a pasted line sits in the composer as text; only a typed Enter submits.
