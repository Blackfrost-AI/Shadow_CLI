...
> Current build: **`v8.6.0`** — the web console as a proper endpoint harness: an Endpoints pane with pinned permanent hosts and a port-resolver doctor, quick-pick probes, an opt-in bounded LAN scan, and one-click “Use as active” promotion; saved connections editable in place with Test endpoint / Test response checks; source-faithful draft copying (`Ctrl+X C`) and display-column caret navigation in the terminal composer; and a self-hosting guide with a no-network provider doctor. On top of the v8.5.0 line: onboarding model discovery (the wizard probes your endpoint and saves your allowlist as `/model` presets — terminal and web) plus the deep-review hardening line, and the v8.4.0 UX parity line (first-run auto-onboard, Shift+Tab plan mode, Ctrl+X M model picker, `/export html`, LSP diagnostics after writes, `/goal` mission mode). Paste-safe input since v8.0.2: a pasted line sits in the composer as text; only a typed Enter submits.
...
- **🔌 The web console is now a proper endpoint harness.** A new **Endpoints pane** (Settings) discovers and verifies API + self-hosted inference servers *before* a preset exists: **pin permanent hosts** whose port is discovered — not remembered — by a resolver doctor that reports whatever is running right now (backend, latency, served models with context windows); probe any URL or click a **quick-pick** (Ollama, llama.cpp, vLLM, LM Studio, SGLang); run an **opt-in bounded LAN scan** (private subnets, known inference ports, host cap + deadline); and promote a hit with one explicit **Use as active** click. Every probe runs server-side through Shadow's egress broker — the browser never leaves the console's origin, and dead hosts fail fast with typed, human hints.
- **🧪 Saved connections are editable + testable in place.** Edit a model ID, endpoint or key without deleting the preset (a blank key keeps the current one; changing an endpoint while keeping credentials asks first). **Test endpoint** checks the models API; **Test response** asks, then sends a fixed sample with a 64-token limit — single-flight, 15s timeout, no redirects, and keys or raw provider bodies never reach the browser.
- **📋 Source-faithful composer copying.** `Ctrl+X` then `C` copies a multi-line draft's *source text* — paragraphs and indentation preserved, no terminal gutters or soft-wrap newlines, paste chips expanded, known secrets redacted. Arrow navigation now counts terminal display columns, so emoji and CJK keep the caret on the right line.
- **🖥 Self-hosting, first-class.** A new `docs/SELF_HOSTING.md` guide (connect a model you already run, honest discovery, doctor workflow) and a no-network provider doctor check that inspects your selected model, credentials and managed local backends without contacting anything.

Earlier highlights:

- **v8.5 — discovery + hardening:** onboarding model discovery (the wizard probes your endpoint and saves your allowlist as `/model` presets), the deep-review hardening line (25 adversarially-verified findings fixed), and the correctness sweep.
