...
> Current build: **`v8.4.0`** — the UX parity line (opencode/OMP gap batch): first-run auto-onboard, Shift+Tab plan mode, Ctrl+X M model picker, `/export html`, stable `web.token`, auto-format after writes, Claude-session import, spend guardrails — plus LSP diagnostics after writes (tsserver/pyright/gopls/rust-analyzer, never blocking a write) and `/goal` mission mode (plan → tasks → verify, HUD-pinned). On top of the v8.0.0 web console line: T1 toasts + context gauge (8.1.0), stream resilience (8.2.0), self-documenting config + instruction-file autopilot (8.3.x). Paste-safe input since v8.0.2: a pasted line sits in the composer as text; only a typed Enter submits.
